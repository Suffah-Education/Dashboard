...
const Editteacherprofile = () => {
  ...
  return (
    <div className="bg-[#f8f9f8] min-h-screen p-6">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-gray-700 hover:text-green-700 transition-colors bg-white border border-gray-200 rounded-lg px-4 py-2 shadow-sm hover:shadow-md mb-6"
      >
        <ArrowLeft size={18} />
      </button>

      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-md p-6 md:p-8">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Edit Teacher Profile</h2>
        ...
        <div className="flex items-center mb-8 space-x-6">
          ...
          <div>
            <p className="text-gray-700 font-medium">{form.name || "Teacher Name"}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={saving}
            className="flex items-center justify-center bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white font-medium px-6 py-2.5 rounded-lg shadow-sm w-full md:w-auto transition"
          >
          ...
          {msg && (
            <p className={`text-center mt-4 font-medium ${msg.includes("success") || msg.includes("Successfully") ? "text-green-600" : "text-red-600"}`}>
              {msg}
            </p>
          )}
        </form>
